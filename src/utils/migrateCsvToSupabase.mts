import { importFromCSV } from './csvHandler.js';
import { createClient } from '@supabase/supabase-js';
import type { Goal as CsvGoal, Milestone, Dimension } from '../types.js';
import type { Goal as SupabaseGoal } from '../types/goal.js';
...
async function main() {
  ...
  const answer = await rl.question('Do you want to insert these goals into Supabase? (yes/no): ');
  rl.close();
  if (answer.trim().toLowerCase() !== 'yes') {
    console.log('Aborted by user. No data inserted.');
    process.exit(0);
  }

  // Insert in batches (Supabase recommends <= 500 rows per insert)
  ...
  for (let i = 0; i < goalsToInsert.length; i += BATCH_SIZE) {
    ...
    const { data, error } = await supabase.from('goals').insert(batch);
    if (error) {
      ...
      console.log(`Batch ${i / BATCH_SIZE + 1}: Inserted ${batch.length} goals.`);
    }
  }
